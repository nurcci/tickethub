"""Стресс-тест защиты от овербукинга (неделя 5).

Гоняет N параллельных запросов на бронь ОДНОГО И ТОГО ЖЕ места через
Django test.Client — то есть настоящие потоки, настоящий Postgres и
Redis (SETNX) внутри контейнера, без сети наружу. Дополняет
locustfile.py: там — общая картина под нагрузкой, здесь — конкретная,
воспроизводимая цифра "сколько раз при N одновременных попытках
прошло ровно одно бронирование".

Запуск (в терминале, при поднятом docker compose):

    docker compose exec web python manage.py shell < events/stress_test.py
"""
