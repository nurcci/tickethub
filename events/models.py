...
class Order(models.Model):
    """Заказ одного места на конкретное мероприятие.

    HOLD выставляется сразу при выборе места (до подтверждения оплаты) —
    это временная бронь, параллельно подстрахованная блокировкой в Redis
    (появится на неделе 3). UniqueConstraint ниже — это защита от овербукинга
    на уровне БД: даже если два запроса как-то проскочат мимо Redis,
    база физически не даст создать второй активный заказ на то же место.
    """
