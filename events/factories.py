"""factory_boy-фабрики для тестов. Раньше тестовые объекты собирались
руками через фикстуры в каждом test_*.py — с ростом числа сценариев
(особенно для сервисного слоя, неделя 4) это стало дублироваться,
поэтому переносим создание объектов сюда в одно место.
"""
